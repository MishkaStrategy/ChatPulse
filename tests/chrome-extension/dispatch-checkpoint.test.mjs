...
test("dispatch checkpoint survives a concurrent profile/control revision change", () => {
  ...
  assert.equal(checkpoint.continuationCount, 0, "newer control revision must not inherit an old-run counter blindly");
...
test("dispatch checkpoint increments the same run even after a profile edit", () => {
  const base = startChatRun(createChat({ title: "A", url: "https://chatgpt.com/c/a" }), {
    at: "2026-09-02T06:00:00.000Z"
  });
  const latest = {
  ...
  // A profile edit changes controlRevision but preserves runStartedAt, so the send belongs to the same run.
  // The model intentionally preserves the fresh profile/revision and always persists the command fingerprint.
  const checkpoint = mergeDispatchCheckpoint(runtime, latest);
